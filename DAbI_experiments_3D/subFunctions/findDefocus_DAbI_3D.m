function [z] = findDefocus_DAbI_3D(imlow_defocus, k_illu, CTF, na_illu, na_obj, wavelength, mag, dpix_c, sub_pixel_resolve, use_GPU)
...
locs   = locs(locs == center | ismembertol(2*center - locs, locs, 2, 'DataScale', 1)); % Filter out all insymmetric peaks
...
if length(locs) < 3 % If insufficient peaks, relax peak detection criteria
    [~, locs] = findpeaks(central_line, 'MinPeakProminence', max(central_line) * 0.01); % Factors may need to be changed
    locs   = locs(locs == center | ismembertol(2*center - locs, locs, 2, 'DataScale', 1)); % Filter out all insymmetric peaks
    locs = locs(abs(locs - center) > delta_k_thesholds(1));
    locs = [locs; center];
    ...
    if length(locs) < 3 % If still no preaks are found, the defocus level should be really low
        ...
        cand = locs(abs(locs-center)>1 & ismembertol(2*center-locs, locs, 2, 'DataScale', 1));
...
if defocus_level == 3 || defocus_level == 2 
    ...
    for n_multi = 1:nBrightField_multi
        ...
        if n_multi == 1
            overlap_crop = rotated(row1:row2, col1:col2);
            [row1_new, row2_new] = rowRangeCrop(overlap_crop,Nk_est_multi,row1,row2);
            cropH = row2_new - row1_new + 1;
            cropW = col2 - col1 + 1;
            if use_GPU
        ...
        overlap_crop_all(:, :, n_multi) = rotated(row1_new:row2_new, col1:col2);
    end
    

    % Find an estimated value of defocus from the defocus aberration (Without high-order compensation)
    z_d = findDefocus_DAbI_FFT_3D(overlap_crop_all, angles_sep, xsize, dpix_c,z_est,2*z_threshold_low, mag, na_illu, sub_pixel_resolve, z_virtual,defocus_level);
    if isnan(z_d)
        z_d = z_est;
    end
    ...
    z_f = findDefocus_DAbI_Direction_3D( ...
                 Nk_est_up, ...
                 Nk_est_down, ...
                 CTF_vir, ...
                 mycoord_mo(1:2,:), ...
    ...
    CTF_def = CTF .* exp(-1i*sqrt_term*z_f - 1i*(0.5*(kx_coord.^2+ky_coord.^2)./k0.*z_f));      
    ...
    z = findDefocus_DAbI_FFT_3D(overlap_crop_all, angles_sep, xsize, dpix_c,z_est,2*z_threshold_low, mag, na_illu, sub_pixel_resolve, z_virtual,defocus_level);
    z = abs(z)*sign(z_f);
    if isnan(z)
        if isnan(z_f)
            z = z_est;
...
%% Part 3-2: Accurate defocus estimation; Condition 2: Small defocus level
    ...
    Nk_est_multi =  xsize * dpix_c / z_virtual / mag / na_illu / sin(angles_sep(1))/2;
    z_est = 0;
    [kx_coord, ky_coord] = meshgrid( linspace(-kmax, kmax - dk, xsize), ...
                                 linspace(-kmax, kmax - dk, xsize) );
    sq_term = k0^2 - kx_coord.^2 - ky_coord.^2;
    ...
    CTF_def = CTF .* exp(-1i*sqrt_term*z_d - 1i*(0.5*(kx_coord.^2+ky_coord.^2)./k0.*z_d)); 
    for i = 1:nBrightField/2
        idx1 = 2*i - 1;
        idx2 = 2*i;
