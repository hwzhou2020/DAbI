function z = findDefocus_DAbI_FFT_3D(overlap_crop_all, angles_sep, xsize, dpix_c, z_est, z_threshold_low, mag, na_illu,na_obj, sub_pixel_resolve, z_shift, defocus_level)
...
%       z_est               - Estimate defocus distance
...
%
%   Output:
%       z                   - Estimated defocus distance (absolute value, um)
...
z_values = [];  % Store z estimates from each pair
...
for n_multi = 1:nBrightField_multi
    ...
    % Pad vertically to 0 rows (for stable performance)
    pad_size = 0 - size(overlap_crop, 1); % May need to change 0 to 512 for higher sampling
    if pad_size > 0
    ...
    central_line = mean(spectrum_pad(:, floor(end/2)-2 : floor(end/2)+2), 2);
    ...
    if z_shift == 0 && defocus_level == 3 % large defocus distance
        Nk = xsize * dpix_c / max(z_est*0.5,0) / mag / na_illu / sin(angles_sep(1))/2;
        delta_k_theshold_down = 1 / Nk * size(spectrum_pad, 1);
        Nk = xsize * dpix_c / (z_est*1.5) / mag / na_illu / sin(angles_sep(1))/2;
    ...
    if length(locs) < 3 % If insufficient peaks, relax peak detection criteria
        [~, locs] = findpeaks(central_line, 'MinPeakProminence', max(central_line) * 0.005);
        locs   = locs(locs == center | ismembertol(2*center - locs, locs, insymmetric_tolerance, 'DataScale', 1));
        if z_shift == 0 && defocus_level == 3    %large defocus distance
            Nk = xsize * dpix_c / max(z_est*0.5,0) / mag / na_illu / sin(angles_sep(1))/2;
            delta_k_theshold_down = 1 / Nk * size(spectrum_pad, 1);
            Nk = xsize * dpix_c / (z_est*1.5) / mag / na_illu / sin(angles_sep(1))/2;
        ...
        if length(locs) >= 3 
            ...
            if sub_pixel_resolve
                loc_p_1 = locs(2);
                loc_p_2 = locs(3);
                loc_p_1_sub = subPixelFit(central_line, loc_p_1, 7, 0.1);
                loc_p_2_sub = subPixelFit(central_line, loc_p_2, 7, 0.1);
