function [z] = findDefocus_DAbI_3D(imlow_defocus, k_illu, CTF, na_illu, na_obj, wavelength, mag, dpix_c, sub_pixel_resolve, use_GPU, precision, max_iter)
    ...
    if length(locs) < 3 % If insufficient peaks, relax peak detection criteria
        [~, locs] = findpeaks(central_line, 'MinPeakProminence', max(central_line) * 0.015); % Factors may need to be changed
        locs   = locs(locs == center | ismembertol(2*center - locs, locs, insymmetric_tolerance, 'DataScale', 1)); % Filter out all insymmetric peaks
        locs = locs(abs(locs - center) > delta_k_thesholds(1));
        locs = [locs; center];
    ...
    if defocus_level == 3 || defocus_level == 2 
        ...
        z_d = findDefocus_DAbI_FFT_3D(overlap_crop_all, angles_sep, xsize, dpix_c,z_est,2*z_threshold_low, mag, na_illu,na_obj, sub_pixel_resolve, z_virtual,defocus_level);
        if isnan(z_d)
            z_d = z_est;
        end
        ...
        while (abs(z_0-z_1) > precision && count < max_iter) || count==1
            ...
            z_2 = findDefocus_DAbI_FFT_3D(overlap_crop_all, angles_sep, xsize, dpix_c,z_est,2*z_threshold_low, mag, na_illu,na_obj, sub_pixel_resolve, z_virtual,defocus_level);
    ...
    %% Part 3-2: Accurate defocus estimation; Condition 2: Small defocus level
        ...
        % Add the known, large defocus aberration to each spectrum
        for i = 1:nBrightField_multi
            idx1 = 2*i - 1;
            idx2 = 2*i;
