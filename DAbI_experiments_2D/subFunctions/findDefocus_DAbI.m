...
% Define thresholds for different defocus levels (in microns)
z_threshold_low = HDoF * 6; % Small
z_threshold_medium = HDoF * 20;  % Medium
z_threshold_high = HDoF * 100;  % Large

% Virtual defocus distance added for low-defocus cases (z_v in Supplementary Note 1.4)
z_virtual = HDoF * 50;
...
mycoord_mo_or = k_illu(multiplex_order, :);
mycoord_mo = round(k_illu(multiplex_order, :));

nBrightField_multi = nBrightField/2; % # of image pairs to use.
...
angles = atan2(-mycoord_mo_or(:, 1), -mycoord_mo_or(:, 2));
angles_sep = abs(angles(2:2:end) - angles(1:2:end))/2;
k_center = (mycoord_mo_or(1:2:end, :) + mycoord_mo_or(2:2:end, :)) / 2;
angles_rad = atan2(-k_center(:, 1), -k_center(:, 2)) / pi * 180;


%% Part 2: Rough estimation of defocus level using FFT peak spacing

% Crop a sub rectangular region in S to estimate the defocus level
crop_ratio_in = 1 / 30;
crop_ratio_out = 1 / 10;
...
SNR_tolerance = 2.5; % May need to be modified according to image SNR
...
overlap_spectrum_unmasked = imrotate(abs(imStack_FFT(:, :, 1)), angles_rad(1), 'bilinear', 'crop');
overlap_crop = overlap_spectrum_unmasked(row1:row2, col1:col2);
...
if length(locs) < 3 % If insufficient peaks, relax peak detection criteria
    [~, locs] = findpeaks(central_line, 'MinPeakProminence', max(central_line) * 0.02); % Factors may need to be changed
    locs   = locs(locs == center | ismembertol(2*center - locs, locs, 3, 'DataScale', 1)); % Filter out all insymmetric peaks
    locs = locs(abs(locs - center) > delta_k_thesholds(1));
    locs = [locs; center];
...
if (defocus_level ~= 3) || (delta_k < 3 * delta_k_thesholds(3))
    crop_ratio_in = 1 / 10;
    crop_ratio_out = 1 / 10;
    % Crop S and rotate to rotate it to align the fringes horizontally
    overlap_spectrum = imrotate(abs(imStack_FFT(:, :, 1)) .* measurement_mask(:, :, 1), angles_rad(1), 'bilinear', 'crop');
...
%% Part 2: Accurate defocus estimation; Condition 1: Large defocus level
if defocus_level == 3 || defocus_level == 2 
    ...
    z_d = findDefocus_DAbI_FFT(overlap_crop_all, angles_sep, xsize, dpix_c,z_est,z_threshold_low, mag, na_illu, sub_pixel_resolve, z_virtual,defocus_level);
    if isnan(z_d)
        z_d = z_est;
    end
    ...
    % Compensate for the higher order aberration terms from defocus. (From the expansion of sqrt(k0^2-kx^2-ky^2)
    % Here, we only iterate once, given that we only use moderate object NA(0.4 NA). Please refer to Supplementary Note 4 for more details. 
    
    z_0 = z_f;
    z_1 = z_f;
    count = 1;
    % Calcuate and compensate for these higher order aberration terms
    while (abs(z_0-z_1) > precision && count < max_iter) || count==1
        CTF_def = CTF .* exp(-1i*sqrt_term*z_1 - 1i*(0.5*(kx_coord.^2+ky_coord.^2)./k0.*z_1));      
        for i = 1:nBrightField/2
            idx1 = 2*i - 1;
            idx2 = 2*i;
        ...
        for n_multi = 1:nBrightField_multi
            rotated = imrotate(abs(imStack_FFT_shift(:, :, n_multi)), ...
                               angles_rad(n_multi), 'bilinear', 'crop');
            overlap_crop_all(:, :, n_multi) = rotated(row1:row2, col1:col2);
        end
        % Recalculate the defocus distance after compensating the higher-order aberration terms
        z_2 = findDefocus_DAbI_FFT(overlap_crop_all, angles_sep, xsize, dpix_c,z_est,z_threshold_low, mag, na_illu, sub_pixel_resolve, z_virtual,defocus_level);
...
%% Part 2: Accurate defocus estimation; Condition 2: Small defocus level
else
    % If the defocus level is small, the fringes are not dense enough for us to accurately estimate defocus distance. 
    % Therefore, we will virtually add a known, large defocus aberration to the spectra.
    % Calculate virtual defocus aberration
    z_est = 0;
    [kx_coord, ky_coord] = meshgrid( linspace(-kmax, kmax - dk, xsize), ...
                                 linspace(-kmax, kmax - dk, xsize) );
    sq_term = k0^2 - kx_coord.^2 - ky_coord.^2;
    ...
    % Add the known, large defocus aberration to the spectra
    for i = 1:nBrightField/2
        idx1 = 2*i - 1;
        idx2 = 2*i;
    ...
    % Compensate for the higher order spherical aberration
    z_0 = z_d;
    z_1 = z_d;
    count = 1;
    while (abs(z_0-z_1) > precision && count < max_iter) || count==1
