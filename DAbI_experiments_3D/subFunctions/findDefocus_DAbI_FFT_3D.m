function z = findDefocus_DAbI_FFT_3D(overlap_crop_all, angles_sep, xsize, dpix_c, z_est, z_threshold_low, mag, na_illu, sub_pixel_resolve, z_shift, defocus_level)
...
for n_multi = 1:nBrightField_multi
    ...
    if z_shift == 0 && defocus_level == 3 %large defocus distance
        Nk = xsize * dpix_c / max(z_est*0.8,0) / mag / na_illu / sin(angles_sep(1))/2;
    ...
    if length(locs) < 3 % If insufficient peaks, relax peak detection criteria
        ...
        locs   = locs(locs == center | ismembertol(2*center - locs, locs, 2, 'DataScale', 1));
        if z_shift == 0 && defocus_level == 3    %large defocus distance
            Nk = xsize * dpix_c / max(z_est*0.8,0) / mag / na_illu / sin(angles_sep(1))/2;
            delta_k_theshold_down = 1 / Nk * size(spectrum_pad, 1);
            Nk = xsize * dpix_c / (z_est*1.2) / mag / na_illu / sin(angles_sep(1))/2;
